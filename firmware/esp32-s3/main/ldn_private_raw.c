...
#error "Private raw TX is audited for the pinned ESP32-C6 / ESP32-S3 drivers"
#endif
...
#else
    /* S3: the driver's TX descriptor layout is not audited; skip the read-only
       trace (the C6 byte offsets would be out of bounds) and just pass through. */
    portENTER_CRITICAL(&s_authorized_lock);
    ++s_descriptor_count;
    portEXIT_CRITICAL(&s_authorized_lock);
#endif
